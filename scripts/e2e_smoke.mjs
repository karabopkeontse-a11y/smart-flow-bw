...
try {
  ...
  console.log('SAVE_PRE', await page.evaluate(() => ({addTask:typeof window.addTask, buttons:[...document.querySelectorAll('button')].filter(b=>b.textContent.includes('Add action')).map(b=>b.textContent.trim())})));
  await page.getByRole('button',{name:'+ Add action',exact:true}).click(); await wait(250);
  console.log('SAVE_POST', await page.evaluate(() => ({modal:!!document.getElementById('sfRuntimeModal'), modalClass:document.getElementById('sfRuntimeModal')?.className, addTask:typeof window.addTask})));
  ...
  await page.locator('#sfAccountBtn').click(); await wait(100); await page.locator('button:has-text("Sign up")').click(); await wait(100); const email=`e2e-${Date.now()}@smartflow.local`;
  await page.locator('input[name="name"]').fill('E2E Household User'); await page.locator('input[name="email"]').fill(email); await page.locator('input[name="password"]').fill('password123'); await page.locator('.sf-box button:has-text("Create account")').click(); await wait(300); if((await page.evaluate(()=>JSON.parse(localStorage.getItem('sf.session')||'null')))?.email!==email) fail('Signup did not establish session');
  await page.locator('#sfAccountBtn').click(); await wait(100); if((await page.locator('.sf-box').innerText()).includes('Administrator</button>')) fail('Account surface exposes duplicate administrator sign-in');
  await page.locator('button:has-text("Households")').click(); await wait(100); await page.locator('button:has-text("Add household")').click(); await wait(100); await page.locator('input[name="name"]').fill('E2E Home'); await page.locator('input[name="address"]').fill('Gaborone'); await page.locator('.sf-box button:has-text("Create household")').click(); await wait(250);
  await page.locator('[data-tab="devices"]').click(); await wait(250); if(!(await page.locator('#sfDevicePanel').innerText()).includes('E2E Home')) fail('Household was not surfaced in Devices');
  await page.locator('#sfDevicePanel button:has-text("Add device")').click(); await wait(100); await page.locator('input[name="name"]').fill('E2E Kitchen Meter'); await page.locator('input[name="location"]').fill('Kitchen'); await page.locator('.sf-box button:has-text("Add device")').click(); await wait(250);
  const deviceText=await page.locator('#sfDevicePanel').innerText(); if(!deviceText.includes('E2E Kitchen Meter') || !deviceText.includes('E2E Home')) fail('Device was not grouped under the selected household');
  await page.locator('#sfDevicePanel button:has-text("＋ Reading")').click(); await wait(100); await page.locator('.sf-box button:has-text("Save reading")').click(); await wait(250); if(!(await page.locator('#sfDevicePanel').innerText()).includes('E2E Kitchen Meter')) fail('Device reading flow failed');
  if(errors.length) fail(errors.join('\n')); console.log('Smart Flow BW E2E smoke test: PASS'); console.log('Simulation, Thothi prompts/answers, Save Water CRUD, account, household, device and reading flows passed.'); await browser.close();
